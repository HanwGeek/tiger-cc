...
//* Estimate whether to swap between a stm and exp or not
static bool commute(T_stm x, T_exp y);
//* Extract ESEQ from T_stm
static T_stm do_stm(T_stm stm);
//* Extract ESEQ from T_exp
...
static expRefList get_call_rlist(T_exp exp) {
  expRefList cur, rlist = cur = ExpRefList(&exp->u.CALL.fun, NULL);
  T_expList args = exp->u.CALL.args;
  for (; args; args = args->tail)
    cur = cur->tail = ExpRefList(&args->head, NULL);
  return rlist;
}
    ...
    *rlist->head == T_Eseq(T_Move(T_Temp(t), *rlist->head), T_Temp(t)); // Store value in temp
    ...
    if (commute(s, x.e)) {
      ...
      *rlist->head = x.e;
      ...
      Temp_temp t = Temp_newtemp();
      *rlist->head = T_Temp(t);
      return seq(x.s, seq(T_Move(T_Temp(t), x.e), s));
    }
  }
...
static void trace(T_stmList list) {
  ...
  } else if (s->kind == T_CJUMP) { /* we want false label to follow CJUMP */
    T_stmList true =  (T_stmList) S_look(block_env, s->u.CJUMP.truee);
    T_stmList false =  (T_stmList) S_look(block_env, s->u.CJUMP.falsee);
    if (false) {
      ...
      last->tail->head = T_Cjump(T_notRel(s->u.CJUMP.op), s->u.CJUMP.left,
				 s->u.CJUMP.right, s->u.CJUMP.falsee, 
				 s->u.CJUMP.truee);
      last->tail->tail = true;
      trace(true);
    } else {
      ...
      last->tail->head = T_Cjump(s->u.CJUMP.op, s->u.CJUMP.left,
				 s->u.CJUMP.right, s->u.CJUMP.truee, false);
