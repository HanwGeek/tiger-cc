 ...
 * @Description: Abstract tree constructor module.
 * @Date: 2019-10-21 16:48:34
 * @Last Modified: 2020-01-08 21:15:49
 */
#include "util.h"
#include "symbol.h"
#include "absyn.h"
...
A_var A_SubscriptVar(A_pos pos, A_var var, A_exp exp) {
  A_var p = checked_malloc(sizeof(*p));
  p->pos = pos; p->kind = A_subscriptVar;
...
A_exp A_VarExp(A_pos pos, A_var var) {
  ...
  p->pos = pos; p->kind = A_varExp;
  p->u.var = var;
  return p;
}
A_exp A_NilExp(A_pos pos) {
  A_exp p = checked_malloc(sizeof(*p));
  p->pos = pos; p->kind = A_nilExp;
  return p;
}

A_exp A_IntExp(A_pos pos, int i) {
  A_exp p = checked_malloc(sizeof(*p));
  p->pos = pos; p->kind = A_intExp;
  p->u.intt = i;
  return p;
}
...
A_exp A_BreakExp(A_pos pos) {
  A_exp p = checked_malloc(sizeof(*p));
  p->pos = pos; p->kind = A_breakExp;
  return p;
}

A_exp A_LetExp(A_pos pos, A_decList decs, A_exp body) {
  A_exp p = checked_malloc(sizeof(*p));
  p->pos = pos; p->kind = A_letExp;
  p->u.let.decs = decs;
  p->u.let.body = body;
  return p;
}
...
A_dec A_FunctionDec(A_pos pos, A_fundecList function) {
  ...
  p->pos = pos; p->kind = A_functionDec;
  p->u.function = function;
  return p;
}
A_dec A_VarDec(A_pos pos, S_symbol var, S_symbol typ, A_exp init) {
  A_dec p = checked_malloc(sizeof(*p));
  p->pos = pos; p->kind = A_varDec;
  p->u.var.var = var;
  p->u.var.typ = typ;
  p->u.var.init = init;
  p->u.var.escape = TRUE;
...
A_fundec A_Fundec(A_pos pos, S_symbol name, A_fieldList params, S_symbol result, A_exp body) {
  ...
  p->pos = pos;
  p->name = name; p->params = params;
  p->result = result; p->body = body;
  return p;
}
