...
T_stm T_Seq(T_stm left, T_stm right) {
  ...
  p->kind = T_SEQ;
  p->u.SEQ.left = left; p->u.SEQ.right = right;
  return p;  
}
...
T_stm T_Jump(T_exp exp, Temp_labelList labels) {
  T_stm p = checked_malloc(sizeof(*p));
  p->kind = T_JUMP;
  p->u.JUMP.exp = exp; p->u.JUMP.jumps = labels;
  return p;
}
...
T_stm T_Cjump(T_relOp op, T_exp left, T_exp right, Temp_label truee, Temp_label falsee) {
  ...
  p->kind = T_CJUMP;
  p->u.CJUMP.op = op;
  p->u.CJUMP.left = left; p->u.CJUMP.right = right;
  p->u.CJUMP.falsee = falsee; p->u.CJUMP.truee = truee;
  return p;
}
...
T_stm T_Move(T_exp dst, T_exp src) {
  T_stm p = checked_malloc(sizeof(*p));
  p->kind = T_MOVE;
  p->u.MOVE.dst = dst; p->u.MOVE.src = src;
  return p;
}
...
T_exp T_Binop(T_binOp op, T_exp left, T_exp right) {
  ...
  p->kind = T_BINOP;
  p->u.BINOP.op = op;
  p->u.BINOP.left = left; p->u.BINOP.right = right;
  return p;
}
...
T_exp T_Eseq(T_stm stm, T_exp exp) {
  T_exp p = checked_malloc(sizeof(*p));
  p->kind = T_ESEQ;
  p->u.ESEQ.stm = stm; p->u.ESEQ.exp = exp;
  return p;
}
...
T_exp T_Call(T_exp fun, T_expList args) {
  T_exp p = checked_malloc(sizeof(*p));
  p->kind = T_CALL;
  p->u.CALL.fun = fun; p->u.CALL.args = args;
  return p;
}
